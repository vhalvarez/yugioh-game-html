class Deck {
	constructor() {
		...
		this.apiUrl ='https://db.ygoprodeck.com/api/v7/cardinfo.php?type=normal%20monster&level=4';
		this.cards = [];
		this.decks = {
			yugi: [],
			player: [],
		...
		this.atkButton = document.getElementById('atkButton');
	...
	addAtkPhaseClickHandler = () => {
		document.getElementById('atkButton').addEventListener('click', () => {
			this.atkPhase();
		});
	};
	...
	drop = (event, dropArea) => {
		...
		const isAttackPosition = window.confirm('¿Deseas colocar la carta en posición de ataque?');
	...
	playRandomCard = () => {
		...
		if (!this.hasPlacedCard[player]) {
			console.log('Ya se ha colocado una carta en este turno.');
			return;
		}
		...
		setTimeout(() => {
			// AQUI LO QUE TIENE QUE SACAR ES UNA CARTA DEL DECK QUE LE CORRESPONDE
			const card = self.decks[player].shift();
			...
			if (emptyHandSlot) {
				...
				// Aquí puedes agregar lógica adicional si Yugi tiene la mano llena
			}
		}, 2000);

		// fin colocar carta en mano
	...
	drawCard = (player) => {
		if (this.currentPhase === 'draw') {
			if (this.hasDrawnCard[player]) {
				alert(
					`El jugador ${player} ya ha robado una carta en este turno.`
				);
				return;
			}
	...
	addDeckClickHandler = (player) => {
		if (this.currentPhase === 'draw') {
			this.mainPhaseButton.disabled = true;
			this.atkButton.disabled = true;
			...
			this.atkButton.style.backgroundColor = 'gray';
			...
			if (deckElement) {
				deckElement.addEventListener('click', () => {
					if (this.currentPlayer === 1 && player === 'player') {
						// Solo permitir robar carta si es el turno del jugador
				...
				if (this.currentPlayer === 2 && player === 'yugi') {
					console.log('yugi debio robar una carta');
					// Lógica para que Yugi robe carta (puedes implementarla según tus necesidades)
					// this.drawCard(player);
					// changeTurn();
				}
			}
	...
	atkPhase = () => {
		...
		this.mainPhaseButton.disabled = true;
		this.mainPhaseButton.style.backgroundColor = 'gray';
		this.atkButton.style.backgroundColor = 'green';
	...
	changeTurn = () => {
		...
		this.turnCount++;
		alert(`Es turno del jugador ${this.currentPlayer} - Turno ${this.turnCount}`);
		...
		if (this.currentPlayer === 2) {
			...
			setTimeout(function () {
				...
				alert('Turno de Yugi finalizado automáticamente.');
				this.mainPhaseButton.disabled = true;
				this.atkButton.disabled = true;
				this.endPhaseButton.disabled = true;
